...
class CustomLSTM(nn.Module):
    ...
    def forward(self, x):
        """
        Führt den Vorwärtsdurchlauf durch.

        Parameter:
        - x (Tensor): Eingabetensor mit den Dimensionen (Batch, Sequenzlänge, Eingabedimension).

        Rückgabe:
        - output[:, -1, :] (Tensor): Ausgabe des LSTM für den letzten Zeitschritt (die letzte Sequenzposition).
        - hidden[0] (Tensor): Versteckter Zustand der obersten LSTM-Schicht nach der Verarbeitung der gesamten Sequenz.
        """

        # Falls die Eingabe eine vierdimensionale Eingabe enthält, die unerwartet ist,
        # und eine Dimension in der Sequenzlänge (Dim 1) 1 ist, wird diese Dimension entfernt.
        if x.dim() == 4 and x.size(1) == 1:
            x = x.squeeze(1)

        # Führt den LSTM-Vorwärtsdurchlauf durch und gibt:
        # - `output`: Die vollständige Ausgabe für jeden Zeitschritt.
        # - `hidden`: Tuple der versteckten Zustände und Zellzustände der letzten LSTM-Schicht.
        output, (hidden, _) = self.lstm(x)

        # Gibt nur die Ausgabe des letzten Zeitschritts für die Vorhersage zurück.
        # `output[:, -1, :]` extrahiert die Ausgabe für den letzten Zeitschritt für jede Batch.
        # `hidden[0]` gibt den letzten versteckten Zustand der obersten LSTM-Schicht zurück.
        return output[:, -1, :], hidden[0]
...
class MultiInputLSTMWithGates(nn.Module):
    ...
    def forward(self, Y, X_p, X_n, Index, self_gate):
        ...
        Parameter:
        - Y (Tensor): Hauptsignal-Tensor mit Dimensionen (Batch, Sequenz, hidden_dim).
        - X_p (Tensor): Positives Nebensignal mit Dimensionen (Batch, Sequenz, hidden_dim).
        - X_n (Tensor): Negatives Nebensignal mit Dimensionen (Batch, Sequenz, hidden_dim).
        - Index (Tensor): Zusätzliche Indexinformationen mit Dimensionen (Batch, Sequenz, hidden_dim).
        - self_gate (Tensor): Torsteuerungssignal zur Modulation der Nebensignale, Dimension (Batch, Sequenz, hidden_dim).

        Rückgabe:
        - output (Tensor): Ausgabe des LSTM für die Sequenz.
        - hidden[0] (Tensor): Letzter versteckter Zustand der obersten LSTM-Schicht.
        """

        # Wenn Eingaben zweidimensional sind, füge eine zusätzliche Dimension hinzu, sodass sie dem Format (Batch, Sequenz, hidden_dim) entsprechen.
        Y = Y.unsqueeze(1) if Y.dim() == 2 else Y
        X_p = X_p.unsqueeze(1) if X_p.dim() == 2 else X_p
        X_n = X_n.unsqueeze(1) if X_n.dim() == 2 else X_n
        Index = Index.unsqueeze(1) if Index.dim() == 2 else Index

        # Anwenden des Tors: Modulation der Nebensignale X_p, X_n und Index durch das self_gate.
        # self_gate steuert, wie stark die Nebensignale zur Ausgabe beitragen.
        X_p = X_p * self_gate
        X_n = X_n * self_gate
        Index = Index * self_gate

        # Kombinieren der Eingaben in einer einzigen Sequenz entlang der letzten Dimension (Feature-Dimension).
        # torch.cat((Y, X_p, X_n, Index), dim=2) erstellt einen Tensor, der alle Eingaben integriert.
        concat_input = torch.cat((Y, X_p, X_n, Index), dim=2)

        # LSTM-Vorwärtsdurchlauf auf den kombinierten Eingang anwenden.
        # - output: Ausgabe für jeden Zeitschritt der Sequenz.
        # - hidden: Versteckter Zustand und Zellzustand der letzten LSTM-Schicht.
        output, (hidden, _) = self.lstm(concat_input)

        # Rückgabe:
        # - Die vollständige LSTM-Ausgabe `output` für jeden Zeitschritt.
        # - Der letzte versteckte Zustand `hidden[0]` der obersten LSTM-Schicht für die Vorhersage.
...
class DualAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Aggregation über die Zeitdimension, um eine verdichtete Ausgabe zu erhalten
