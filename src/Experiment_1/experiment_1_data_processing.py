...
def generate_sequences(data, sequence_length=50, forecast_steps=30, target_feature_dim=10):
    ...
    for i in range(len(data) - sequence_length - forecast_steps):
        ...
        # Zusicherung der richtigen Spaltenanzahl und gleichmäßige Dimensionierung
    ...
    print(f"Debug: Final X shape {X.shape}, Y shape {Y.shape}")  # Sollten beide [Batch, Seq, target_feature_dim] haben
...
if __name__ == "__main__":
    ...
    indicator_group_1 = scaled_df[['rsi', 'macd', 'macd_signal', 'momentum', 'stochastic']]
    indicator_group_2 = scaled_df[['cci', 'roc', 'bb_upper', 'bb_lower']]
