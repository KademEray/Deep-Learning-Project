...
def build_multistep_model(input_shape, output_steps, learning_rate=0.0001):
    ...
    # LSTM layers with BatchNormalization and Dropout for regularization
    model.add(LSTM(512, return_sequences=True, input_shape=input_shape))  # 1. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.5))

    model.add(LSTM(512, return_sequences=True))  # 2. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.5))

    model.add(LSTM(256, return_sequences=True))  # 3. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.5))

    model.add(LSTM(256, return_sequences=True))  # 4. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.5))

    model.add(LSTM(128, return_sequences=True))  # 5. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.4))

    model.add(LSTM(128, return_sequences=True))  # 6. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.4))

    model.add(LSTM(64, return_sequences=True))  # 7. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.4))

    model.add(LSTM(64, return_sequences=True))  # 8. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.4))

    model.add(LSTM(32, return_sequences=True))  # 9. LSTM-Schicht
    model.add(BatchNormalization())
    model.add(Dropout(0.4))

    model.add(LSTM(32))  # 10. LSTM-Schicht, die letzte Schicht hat kein return_sequences=True
    model.add(BatchNormalization())
    model.add(Dropout(0.4))
...
def train_and_save_model(X_train, y_train, X_val, y_val, model_name, output_steps, epochs=200, batch_size=256,
    ...
    model_directory = "../../models/extreme_model"
...
if __name__ == "__main__":
    model_directory = "../../models/extreme_model"
    model_name = "sol_extreme_model.h5"
    ...
    if os.path.exists(model_path):
        ...
        if user_input == 'n':
            ...
            train_and_save_model(X_train, y_train, X_val, y_val, model_name="sol_extreme_model", output_steps=30)
