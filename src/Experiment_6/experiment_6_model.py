...
import pickle
from experiment_6_model_layer import CustomLSTM, MultiInputLSTMWithGates, DualAttention
...
class SimpleCNN(nn.Module):
    def __init__(self, input_dim, seq_length, output_dim):
        ...
        self.fc = nn.Linear(64 * seq_length, output_dim)
        self.relu = nn.ReLU()

    def forward(self, x):
        # Erwarte Eingabe (Batch, Seq, Features) und transponiere sie für Conv1d
        x = x.transpose(1, 2)  # (Batch, Features, Seq)
        x = self.relu(self.conv1(x))
        x = self.relu(self.conv2(x))
        x = x.view(x.size(0), -1)  # Flach für den Fully Connected Layer
        x = self.fc(x)
        return x
