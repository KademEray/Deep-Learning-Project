...
class Autoencoder(nn.Module):
    """
    A simple Autoencoder neural network with one hidden layer for encoding and decoding.
    Args:
    ...
    Attributes:
        ...
        decoder (nn.Sequential): The decoder part of the autoencoder, which reconstructs the input data from the encoded representation.
    Methods:
        forward(x):
            Encodes the input data and then decodes it back to the original dimension.
            Args:
                x (torch.Tensor): The input data.
            Returns:
                encoded (torch.Tensor): The encoded representation of the input data.
                decoded (torch.Tensor): The reconstructed input data from the encoded representation.
    """
...
def generate_sequences(data, sequence_length=50, forecast_steps=30):
    ...
    Returns:
        tuple: A tuple containing two torch tensors:
            ...
            - Y (torch.Tensor): The target sequences of shape (num_sequences, forecast_steps, num_features).
    """
    X, Y = [], []
...
if __name__ == "__main__":
    ...
    # Close-Daten der zusätzlichen Kryptowährungen hinzufügen
    for crypto in additional_cryptos:
        crypto_data = yf.download(crypto, start=start_date, end=end_date)
        df[f'{crypto}_close'] = crypto_data['Close']
    ...
    indicator_group_1 = scaled_df[
        ['rsi', 'macd', 'macd_signal', 'momentum', 'stochastic', 'adx', 'willr', 'cci20', 'mfi', 'cci', 'roc',
         'bb_upper', 'bb_lower', 'sma20', 'ema50', 'upperband', 'lowerband', 'obv']]
    indicator_group_2 = scaled_df[[f'{crypto}_close' for crypto in additional_cryptos]]
